...
abstract class Upload {
	...
	public $result; // хранение результата
	public $json; // хранение json
	...
	public function __construct(){
		...
		$this->result = $done_files ? array ('files' => $files, 'info' => $done_files) : array ('error' => 'Ошибка загрузки файлов');
		$this->json = json_encode ($result); // Конвертируем полученный массив данных в json формат
	}

	// Функция, возвращающая ассоциативный массив с сообщением об успешной записи файла и сопутствующие данные
	public function msg ($uploadPath, $name, $format) {
		return array (
			'status' => 'Sucess!',
	...
	public function load ($filePath, $errorCode, $uploadPath = 'pics') {
		...
		if ($errorCode !== UPLOAD_ERR_OK || !is_uploaded_file ($filePath)) {
			...
			return $this->error ($outputMessage); // Выведем название ошибки
		}
		...
		if (strpos ($mime, 'image') === false) return $this->error ('Можно загружать только изображения.');
		$str = implode (", ", $formats); // Проверим форматы image (jpeg, png и т. д.)
		if (strpos ($str, explode ('/', $mime)[1]) === false) return $this->error ("Можно загружать только изображения в форматах: $str");

		// Проверим нужные параметры
		if (filesize ($filePath) > $limitBytes) return $this->error ("Размер изображения не должен превышать $limitBytes Мбайт.");

		$image = getimagesize ($filePath); // Результат функции запишем в переменную

		//$limitWidth  = 2000;
		//$limitHeight = 1500;
		//if ($image[0] > $limitWidth) return $this->error ("Ширина изображения не должна превышать $limitWidth точек.");
		//if ($image[1] > $limitHeight) return $this->error ("Высота изображения не должна превышать $limitHeight точек.");
		...
		if (move_uploaded_file ($filePath, WWW . "/$uploadPath/$name.$format")) return $this->msg ($uploadPath, $name, $format);
		else return $this->error ('При записи изображения на диск произошла ошибка.');
	}
