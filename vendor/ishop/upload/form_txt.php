...
if (isset ($_FILES['file']) && isset ($_FILES['file-2'])) {
    ...
    $check_2 = can_upload ($_FILES['file-2']);

    // запись в файл
    date_default_timezone_set ('Europe/Moscow');
    $today = date ('Y-m-d H:i:s');
    $filename = 'img/file-1.txt';
    ...
    if (!empty ($name) && !empty ($surname) && !empty ($rang)) {
        if (!is_dir ('img')) mkdir ('img', 0777); // Создадим директорию, если она отсутсвует
        if (!is_file ($filename)) {
