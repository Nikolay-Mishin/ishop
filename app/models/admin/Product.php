...
class Product extends AppModel {
	...
	public static array $filter; // фильтры
	public static array $related_product; // связанные товары
	public static array $gallery; // галлерея
	...
	public static function getAll(bool $pagination = true, int $perpage = 10): array {
	...
	public function saveGallery(int $id): void {
	...
	public function editAttrs(int $id, array $data, string $table, string $condition, $attr_id): void {
	...
	protected function deleteAttrs(int $id, string $table, string $condition): void {
		\R::exec("DELETE FROM $table WHERE $condition = ?", [$id]); // выполняем sql-запрос
	}

	// метод добавления товара
	protected function addAttrs(int $id, array $data, string $table, string $condition, $attr_id): void {
